...
Public publishing requires an active Clerk account, current terms acceptance, and a package named for the account's Lemonize namespace. GitHub is optional unless an operator explicitly grants an additional GitHub-bound scope. Production remains read-only until the documented cutover gates pass. See the [CLI documentation](https://lemonize.cyou/docs) for authentication, source selection, frozen installs, token commands, and publishing status.
