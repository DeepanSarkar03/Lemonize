...
| `POST`   | `/v1/auth/logout`         | Clerk or API token   | Revokes the presenting API token; a Clerk-session request does not terminate Clerk itself                                                   |
| `POST`   | `/v1/tokens`              | `manage:tokens`      | Creates a 1-90 day token and returns the raw value once; an API token cannot grant scopes it lacks or outlive its parent                    |
| `GET`    | `/v1/tokens`              | `manage:tokens`      | Lists active token metadata and prefixes, never raw values                                                                                  |
| `DELETE` | `/v1/tokens/:id`          | `manage:tokens`      | Revokes one of the caller's tokens                                                                                                          |
...
Supported API-token scopes are `read`, `publish`, `manage:packages`, and `manage:tokens`. A device token lasts 30 days. Consumers receive `read` and `manage:tokens`; GitHub-linked publishers/admins receive all four scopes. `read` is reserved/descriptive today; token authentication validates the stored scope set, while only publish/manage routes enforce a capability scope.
