...
| ------------------------------------------------------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production Clerk instance/custom domain is not provisioned          | There is no production issuer or stable production browser identity path | Create/configure the production instance, activate its custom domain, independently resolve DNS, and verify a production JWT against that exact issuer                       |
| GitHub OAuth is not configured for production Clerk                 | The required public-publisher sign-in route is not ready                 | Create the production OAuth client, configure its secret/callback in Clerk, then test login, logout, account lock, GitHub linking, and device approval                       |
| `npm.lemonize.cyou` is unresolved; WAF access is missing            | The public npm proxy cannot be advertised as a production CDN            | Use credentials with the narrow DNS/WAF scopes absent from the current Cloudflare OAuth session, deploy/resolve the custom domain, apply the rules, and record passing tests |
| npm CLI is signed out and trusted publishing is not configured      | `@lemonize/cli` cannot be released through the protected OIDC workflow   | Authenticate an npm owner, create/verify `@lemonize`, and bind the exact GitHub repository/workflow as a trusted publisher                                                   |
| `CLI_R2_API_TOKEN` is not provisioned                               | The protected CLI release job cannot publish standalone binaries         | Create a production GitHub environment secret whose Cloudflare token can write only CLI release objects in the intended R2 bucket; do not reuse the Worker deployment token  |
| Appwrite CLI has no authenticated deploy key with `functions.write` | Schemas and the checked-in scanner revision cannot be reconciled         | Create a CI-only key with function/schema administration, authenticate the CLI noninteractively, deploy staging/production resources, then exercise clean and rejected scans |
| Protected GitHub environments and required checks are not finalized | A reproducible reviewed production deployment cannot be asserted         | Populate isolated staging/production variables and secrets, require production review, protect `main`, and pass every named CI/deployment gate on the release SHA            |
...
The scanner validates the archive's `package.json` identity and safe paths. The signed scan protocol also compares a canonical SHA-256 digest of the complete archive manifest with the client-declared metadata stored in TablesDB. The checked-in revision containing that gate still needs to be deployed with the required Appwrite deploy credential before production publishing is enabled.
...
`npm.lemonize.cyou` is a separate public, download-only npmjs proxy. It supports package metadata, tarballs, search, ping, and two audit endpoints, but not login, users, tokens, teams, stars, download statistics, replication feeds, or mutations. In free mode, successful packuments larger than 256 KiB (up to the 16 MiB hard cap) stream without tarball-URL rewriting, so those tarballs can download directly from npmjs. Origin Range misses are not stored as partial cache entries. See [npm CDN proxy](NPM_PROXY.md).
