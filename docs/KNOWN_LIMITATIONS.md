...
| Scanner header integration bug and Appwrite build handoff outage | Appwrite 1.9.5 supplies the execution key in `x-appwrite-key`, but the pinned scanner checks environment variables only; the Frankfurt build service can then report `Build produced no output artifact.` while deploying the correction. A temporary unsupported shim supplies a dedicated static scanner key, while deployment may retain only an exactly attested stale scanner; otherwise it fails closed | After a corrected scanner consumes the header and Appwrite reliably hands off that artifact, remove the provider key, protected secret/attestation, secret function variable, static-key shim, version-pinned stale exception, and bounded retry |
...
During the temporary scanner shim, each environment uses a distinct protected
`APPWRITE_SCANNER_API_KEY` that is passed only to the scanner deployment step
and stored as secret function variable `APPWRITE_API_KEY`. Its protected key-ID
and exact, expiring reviewer attestation accompany only that step. It must never
reuse the Worker runtime, CI deploy, or HMAC credential. CI validates that record
and functionally tests `.tgz` create/read/delete in the quarantine bucket, but an
opaque secret cannot be bound cryptographically to its recorded ID or prove the
absence of extra scopes. Retain provider-console evidence for **only**
`files.read` and `files.write`, enforce expiry within 90 days, and revoke replaced
keys. Those grants remain project-wide despite bucket pinning and canaries. Every
scanner deployment requires the live registry to remain exactly read-only and
verifies zero project variables plus an exact seven-variable function allowlist.
