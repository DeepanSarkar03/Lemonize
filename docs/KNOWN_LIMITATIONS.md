...
| Appwrite scanner builds can fail during artifact handoff  | The Frankfurt build service can report `Build produced no output artifact.` after a successful build; deployment may retain only a byte-identical active ready scanner, with a separately reviewed Appwrite-1.9.5 attestation for the two known stale markers, otherwise it retries that exact failure twice before failing closed | Keep the guarded fallback pins current, and remove the version-pinned stale exception and bounded retry after Appwrite restores reliable function artifact output |
