...
| -------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Production Clerk interactive launch drills are incomplete      | Provider DNS, SSL, mail, and GitHub OAuth are live, but the full publisher path is not launch-proven | Test first-user GitHub login, lockout, linking, terms acceptance, and device approval                                                                                       |
...
| npm trusted publishing is not configured                       | `@lemonize/cli@0.1.0` is public, but subsequent protected OIDC releases are blocked                  | Bind `DeepanSarkar03/Lemonize`, `release-cli.yml`, and environment `production` as the package's trusted publisher                                                          |
