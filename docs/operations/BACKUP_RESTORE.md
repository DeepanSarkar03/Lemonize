...
Use the reviewer-gated `.github/workflows/appwrite-restore-drill.yml` workflow for that test. Dispatch it from `main` only after the exact commit has passed all required CI checks, and enter `RESTORE_STAGING_TO_ISOLATED_DATABASE` as the confirmation. The workflow never restores over `registry`: it creates a disabled synthetic staging database with a sentinel row, takes a one-off resource-scoped archive, restores it to a second generated disabled database, verifies the schema and sentinel, confirms the runtime database did not change, and deletes both temporary databases plus the one-off archive. It preserves key separation: the deploy key manages drill schema resources, a staging-only restore-data key has only `rows.read` and `rows.write`, and the backup key manages the one-off archive and restoration. Appwrite row scopes are project-wide, so the script—not the key—pins every data operation to the generated database, table, and row IDs and refuses duplicate key values. The Worker runtime key is never exposed to the drill. All Appwrite-mutating staging workflows share one concurrency lock so schema, scanner, backup, deployment, and restore operations cannot overlap. If an asynchronous create, archive, or restoration operation does not reach a safe terminal state, the workflow fails without racing provider-side work: it prints the exact generated resource IDs and preserves them for reviewed cleanup after Appwrite reports a terminal state. The next drill remains blocked by orphan preflight until that cleanup is complete.
