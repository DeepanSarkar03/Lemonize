...
| Free Bot Fight Mode challenges trusted automation              | GitHub-hosted production probes receive Cloudflare `managed_challenge` responses from `bot_fight_mode`, so registry/CDN monitoring reports HTTP 403 despite healthy Workers | Disable zone-wide free Bot Fight Mode, or upgrade and add exact hostname skips; then rerun the protected production-health workflow            |
...
| npm trusted publishing is not configured                       | `@lemonize/cli@0.1.0` is public; the hardened, provenance-enabled `0.1.3` release is packaged but npm blocks its OIDC upload                                                | Bind `DeepanSarkar03/Lemonize`, `release-cli.yml`, and secret-free environment `npm-release` as the package's trusted publisher                |
