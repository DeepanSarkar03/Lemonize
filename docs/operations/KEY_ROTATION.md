...
- CLI releases use npm trusted publishing and do not retain a package or R2 release token. Review the trusted-publisher binding whenever repository, workflow, or environment identity changes.
...
- Appwrite: rotate `APPWRITE_DEPLOY_API_KEY`, `APPWRITE_RUNTIME_API_KEY`, `APPWRITE_RESTORE_DATA_API_KEY`, and `APPWRITE_BACKUP_API_KEY` independently. The deploy key may administer checked-in schemas/functions but must never enter the Worker; the runtime key is limited to required rows, files, and execution calls; the staging-only restore-data key is limited to `rows.read` and `rows.write`; the backup key is limited to backup operations. Validate project ID matching before revoking a prior key.
...
- Lemonize API tokens: use a root to create a replacement child with only the required non-management scopes, update the consumer, then revoke the old child. Revoke and replace the root through Clerk/device login when rotating the whole lineage; root revocation invalidates all of its children. `read` is currently reserved/descriptive; `publish`, `manage:packages`, and `manage:tokens` are enforced capabilities, but API-created children never receive `manage:tokens`. Creation responses expose the raw token once, and Durable Object device delivery is atomic at-most-once; never copy either into audit records.
